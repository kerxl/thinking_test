...
async def senler_webhook(request: SenlerWebhookRequest):
    ...
    """
    try:
        logger.info(f"Получен webhook от Senler: user_id={request.user_id}, username={request.username}")
...
async def complete_test_for_senler(user_id: int, message: Optional[str] = None):
    ...
    """
    try:
        final_message = message or "Спасибо за прохождение теста! Возвращаемся в Senler..."
        
        success = await senler_integration.return_user_to_senler(user_id, final_message)
        
        if success:
            return {"success": True, "message": "Пользователь успешно возвращен в Senler"}
        else:
            raise HTTPException(status_code=404, detail="Пользователь не найден или нет Senler token")
